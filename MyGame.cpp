...
void MyGame::Update() {
	ImGui::Begin(" ");
	ImGui::SliderInt("2x2Block", &blockNum2x2, 0, 5);  // 最小値0、最大値10
	ImGui::SliderInt("3x3Block", &blockNum3x3, 0, 5);  // 最小値0、最大値10
	ImGui::SliderInt("3x2Block", &holeNum3x2, 0, 5);    // 最小値0、最大値10
	ImGui::SliderInt("4x3Block", &holeNum4x3, 0, 5);    // 最小値0、最大値10
	...
	for (uint32_t y = 0; y < mapField_->kNumBlockVirtical; ++y) {
		for (uint32_t x = 0; x < mapField_->kNumBlockHorizontal; ++x) {
			...
			if (type == MapType::kBlock) {
				// 障害物の描画
				Novice::DrawBox(static_cast<int>(position.x), static_cast<int>(position.y), width, height, 0.0f, 0xaa1111ff, kFillModeSolid);
				...
				// すでにkGoalが存在する場合、それを削除
				mapField_->mapData_[x][y] = static_cast<int>(MapType::kBlank);
			}
		}
	}

	player_->Draw(*mapField_);
}