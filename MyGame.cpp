...
void MyGame::Initialize() {
    mapChipField_ = new MapField();
    mapChipField_->GenerateRandomObstacles(blockNum2x2, blockNum3x3, holeNum3x2, holeNum4x3);

    IndexSet indexSet = { 10, 5 };
    player_ = new Player();
    playerPosition_ = mapChipField_->GetPositionByIndexSet(indexSet);
...
void MyGame::Update() {
    ...
    player_->Update(mousePosition_,point);
}
...
void MyGame::Draw() {
    ...
    for (uint32_t y = 0; y < mapChipField_->GetNumBlockVirtical(); ++y) {
        for (uint32_t x = 0; x < mapChipField_->GetNumBlockHorizontal(); ++x) {
            ...
            Vector2 position = mapChipField_->GetMapChipPositionByIndex(x, y);
